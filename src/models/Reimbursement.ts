...
export function instanceOfReimbursement(value: object): value is Reimbursement {
    ...
    if (!('expenseTitle' in value) || value['expenseTitle'] === undefined) return false;
    if (!('expenseIsProduction' in value) || value['expenseIsProduction'] === undefined) return false;
...
export function ReimbursementToJSONTyped(value?: Omit<Reimbursement, 'id'|'payee_name'|'approver_name'|'expense_title'|'expense_is_production'|'production_id'|'production_name'|'created_at'|'updated_at'|'archive_at'|'tenant'|'created_by'|'updated_by'> | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
