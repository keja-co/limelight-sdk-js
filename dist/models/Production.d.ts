/**
 * Limelight API
 * Limelight manages your theatre production process from auditions to bump out.
 *
 * The version of the OpenAPI document: 1.0.20
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
import type { StatusEnum } from './StatusEnum';
/**
 *
 * @export
 * @interface Production
 */
export interface Production {
    /**
     *
     * @type {number}
     * @memberof Production
     */
    readonly id: number;
    /**
     *
     * @type {Date}
     * @memberof Production
     */
    readonly createdAt: Date;
    /**
     *
     * @type {Date}
     * @memberof Production
     */
    readonly updatedAt: Date;
    /**
     *
     * @type {Date}
     * @memberof Production
     */
    archiveAt?: Date | null;
    /**
     *
     * @type {string}
     * @memberof Production
     */
    name: string;
    /**
     *
     * @type {string}
     * @memberof Production
     */
    description?: string;
    /**
     *
     * @type {Date}
     * @memberof Production
     */
    startDate?: Date | null;
    /**
     *
     * @type {Date}
     * @memberof Production
     */
    endDate?: Date | null;
    /**
     *
     * @type {string}
     * @memberof Production
     */
    venue?: string;
    /**
     *
     * @type {StatusEnum}
     * @memberof Production
     */
    status?: StatusEnum;
    /**
     *
     * @type {number}
     * @memberof Production
     */
    readonly tenantId: number;
    /**
     *
     * @type {number}
     * @memberof Production
     */
    readonly createdBy: number | null;
    /**
     *
     * @type {number}
     * @memberof Production
     */
    readonly updatedBy: number | null;
    /**
     *
     * @type {number}
     * @memberof Production
     */
    director?: number | null;
}
/**
 * Check if a given object implements the Production interface.
 */
export declare function instanceOfProduction(value: object): value is Production;
export declare function ProductionFromJSON(json: any): Production;
export declare function ProductionFromJSONTyped(json: any, ignoreDiscriminator: boolean): Production;
export declare function ProductionToJSON(json: any): Production;
export declare function ProductionToJSONTyped(value?: Omit<Production, 'id' | 'created_at' | 'updated_at' | 'tenant_id' | 'created_by' | 'updated_by'> | null, ignoreDiscriminator?: boolean): any;
