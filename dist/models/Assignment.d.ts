/**
 * Limelight API
 * Limelight manages your theatre production process from auditions to bump out.
 *
 * The version of the OpenAPI document: 1.0.98
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
/**
 *
 * @export
 * @interface Assignment
 */
export interface Assignment {
    /**
     *
     * @type {number}
     * @memberof Assignment
     */
    readonly id: number;
    /**
     *
     * @type {string}
     * @memberof Assignment
     */
    readonly userName: string | null;
    /**
     *
     * @type {string}
     * @memberof Assignment
     */
    readonly productionName: string | null;
    /**
     *
     * @type {Date}
     * @memberof Assignment
     */
    readonly createdAt: Date;
    /**
     *
     * @type {Date}
     * @memberof Assignment
     */
    readonly updatedAt: Date;
    /**
     *
     * @type {Date}
     * @memberof Assignment
     */
    readonly archiveAt: Date | null;
    /**
     * Date and time when the asset was assigned
     * @type {Date}
     * @memberof Assignment
     */
    startDate: Date;
    /**
     * Date and time when the asset was returned
     * @type {Date}
     * @memberof Assignment
     */
    endDate?: Date | null;
    /**
     *
     * @type {number}
     * @memberof Assignment
     */
    readonly tenant: number;
    /**
     *
     * @type {number}
     * @memberof Assignment
     */
    readonly createdBy: number | null;
    /**
     *
     * @type {number}
     * @memberof Assignment
     */
    readonly updatedBy: number | null;
    /**
     * The asset being assigned
     * @type {number}
     * @memberof Assignment
     */
    asset: number;
    /**
     * User to whom the asset is assigned
     * @type {number}
     * @memberof Assignment
     */
    user?: number | null;
    /**
     * Production associated with the asset assignment
     * @type {number}
     * @memberof Assignment
     */
    production?: number | null;
}
/**
 * Check if a given object implements the Assignment interface.
 */
export declare function instanceOfAssignment(value: object): value is Assignment;
export declare function AssignmentFromJSON(json: any): Assignment;
export declare function AssignmentFromJSONTyped(json: any, ignoreDiscriminator: boolean): Assignment;
export declare function AssignmentToJSON(json: any): Assignment;
export declare function AssignmentToJSONTyped(value?: Omit<Assignment, 'id' | 'user_name' | 'production_name' | 'created_at' | 'updated_at' | 'archive_at' | 'tenant' | 'created_by' | 'updated_by'> | null, ignoreDiscriminator?: boolean): any;
